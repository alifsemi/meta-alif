...
int main()
{
    ...
    fd_semhu1_ept = open("/dev/rpmsg1", O_RDWR | O_CREAT);// | O_NONBLOCK);

    if(fd_semhu1_ept == -1) {
	    printf("Unable to open file /dev/rpmsg1, error code is %d. Trying again!\n", errno);
	    fd_semhu1_ept = open("/dev/rpmsg1", O_RDWR | O_CREAT);
	    if (fd_semhu1_ept == -1) {
