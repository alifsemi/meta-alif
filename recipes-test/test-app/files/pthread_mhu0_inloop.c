...
int main()
{
     ...
     status = ioctl(fd, RPMSG_CREATE_EPT_IOCTL, &es0mhu0_eptinfo);
     if (status == -1) {
    ...
    fd_es0mhu0_ept = open("/dev/rpmsg0", O_RDWR);// | O_NONBLOCK);
...
void * print_message_function_send( void *ptr )
{
	...
	pthread_mutex_lock( &mutex1 );
        status = write(fd_es0mhu0_ept, &data, sizeof(data));
	if(status == -1) {
...
void * print_message_function_receive( void *ptr )
{
	...
	status = read(fd_es0mhu0_ept, &data, sizeof(data));
